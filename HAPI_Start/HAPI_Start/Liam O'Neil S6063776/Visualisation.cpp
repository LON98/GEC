...
void Visualisation::CreateSprite(const std::string & filePath, std::string name)
{
	...
	/*HAPI.UserMessage(std::to_string(sprites.at(name)->getRect().Height()), "Debug Height");
	HAPI.UserMessage(std::to_string(sprites.at(name)->getRect().Width()), "Debug Width");*/
}
...
void Visualisation::ClipBlit(BYTE * screen, const Rectangle & screenRect, std::string name, int posX, int posY)
{
	if (sprites.at(name)->getRect().ContainsComplete(screenRect))
	{
		DrawSprite(name, screen, screenRect.Height(), screenRect.Width(), posX, posY);
			
	}
	else
	{
		Rectangle texRect{ sprites.at(name)->getRect() };
		...
		scrPtr += (posX + posY*screenRect.Width()) * 4;
		texPtr += (clippedRect.left + (clippedRect.top * texRect.Width())) * 4;
		for (int j = 0; j < clippedRect.Height(); j++)
		{
			...
			scrPtr += (screenRect.Width() - clippedRect.Width()) * 4;
			texPtr += (sprites.at(name)->getRect().Width() - clippedRect.Width()) * 4;
